...
namespace {
constexpr uint8_t SWITCH_OUTPUT_PIN = 45;
constexpr uint32_t TEST_PULSE_MS = 5000;
bool switchPulseActive = false;
uint32_t switchPulseEndMs = 0;

bool payloadEquals(const meshtastic_MeshPacket *mp, const char *command)
{
    if (!mp || mp->which_payload_variant != meshtastic_MeshPacket_decoded_tag ||
    ...
    return payload.size == cmdLen && memcmp(payload.bytes, command, cmdLen) == 0;
}

void setSwitchOutput(bool on)
{
    digitalWrite(SWITCH_OUTPUT_PIN, on ? HIGH : LOW);
}
} // namespace
...
int MeshService::handleFromRadio(const meshtastic_MeshPacket *mp)
{
    ...
    // Additive command hook: keep the original packet-forwarding flow unchanged.
    if (payloadEquals(mp, "BAODONG")) {
        setSwitchOutput(true);
        switchPulseActive = false;
    } else if (payloadEquals(mp, "DUNGBAODONG")) {
        setSwitchOutput(false);
        switchPulseActive = false;
    } else if (payloadEquals(mp, "TEST")) {
        setSwitchOutput(true);
        switchPulseActive = true;
        switchPulseEndMs = millis() + TEST_PULSE_MS;
    }
...
{
    if (switchPulseActive && static_cast<int32_t>(millis() - switchPulseEndMs) >= 0) {
        setSwitchOutput(false);
        switchPulseActive = false;
    }
